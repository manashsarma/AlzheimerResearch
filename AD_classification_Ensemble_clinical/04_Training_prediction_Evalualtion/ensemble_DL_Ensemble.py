...
def evaluate_model(X_train, Y_train, testX, testy):
  ...
  model.add(Dense(20, input_dim=8, kernel_initializer='normal', activation='relu', kernel_regularizer=l2(0.0001), bias_regularizer=l2(0.0001)))
  model.add(Dropout(0.2))
  #model.add(Dense(20, kernel_initializer='normal', activation='relu'))
  model.add(Dense(20, kernel_initializer='normal', activation='relu', kernel_regularizer=l2(0.0001), bias_regularizer=l2(0.0001)))
  model.add(Dropout(0.2))
  #model.add(Dense(3, kernel_initializer='normal', activation='softmax'))
  model.add(Dense(3, kernel_initializer='normal', activation='softmax', kernel_regularizer=l2(0.0001), bias_regularizer=l2(0.0001))) 

  # Compile model
  
  adamopt = optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0, amsgrad=False)  
  ...
  # Fit the model
  # use call back for interactive plotting after every epoch
  #history = model.fit(X_train, Y_train,
  #epochs=100, class_weight=class_weight, verbose=0, callbacks=[plot])
  weights = {0:117, 1:100, 2:263}
  #history = model.fit(X_train, one_hot_labels,class_weight=weights, batch_size=5, epochs=200, verbose=1)
...
for train_index , test_index in kf.split(trainX):
    ...
    historyDL, model, test_acc = evaluate_model(X_train, Y_train, X_test, Y_test)
    print('>%.3f' % test_acc)
